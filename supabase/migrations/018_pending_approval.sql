-- 018_pending_approval.sql
-- זרימת אישור: עובד מסיים → ממתין לאישור → מנהל מאשר / מחזיר לתיקון.
-- הגדרה ברמת ארגון: require_approval (ברירת מחדל true).

-- ── ערכים חדשים ל-enums ──────────────────────────────────────────────
alter type task_status add value if not exists 'pending_approval' after 'blocked';
alter type event_type add value if not exists 'pending_approval';
alter type event_type add value if not exists 'approved';
alter type event_type add value if not exists 'returned';
alter type event_type add value if not exists 'transferred';

-- ── הגדרת ארגון ──────────────────────────────────────────────────────
alter table organizations
  add column if not exists require_approval boolean not null default true;

-- ── cancel_task — גם pending_approval ניתן לביטול ───────────────────
-- אין שינוי לוגי — סטטוס cancelled כבר מותר מכל מצב. רק הוספת תיעוד.

-- ── tasks_guard_manager_edit — מנהלים בלבד: approved/returned/transferred ─
-- גם pending_approval (עובד מגיש לאישור) מותר לעובד ← לא חוסמים.
-- אבל מעבר ישיר ל-done על ידי עובד כשיש require_approval — חוסמים.
...
begin
  ...
  -- ביטול משימה — מנהלים בלבד
  if new.status = 'cancelled' and old.status is distinct from 'cancelled' and not v_is_mgr then
    raise exception 'not_manager';
  end if;

  -- עובד לא יכול להעביר ישירות ל-done כשיש חובת אישור
  if new.status = 'done' and old.status is distinct from 'done' and not v_is_mgr then
  ...
  if jsonb_array_length(v_changes) > 0 then
    ...
    values (new.org_id, new.id, v_actor, 'edited', v_changes);
  end if;
